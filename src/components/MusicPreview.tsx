...
export default function MusicPreview({filePath,songAlbum='Lost in Paradise',songArtist=['JAKE',"Mia"],songGenre=['Unknow'],songName,songYear=2000,songSize=10,sontLength=100,fileType='mp3',songImage=""}: prop) {
        ...
        <>
            <div className="flex flex-row justify-between items-center w-full px-4 py-2 rounded hover:border hover:bg-[#ffffff56] ">
                <div className="flex flex-row gap-2 items-center ">
                    ...
                    <img src={songImage!==""? songImage:defaultSongImg} alt="" className="w-10 h-10 rounded-full" />
                    <div className="flex flex-col justify-center">
                        <h3 className="font-bold text-white text-ellipsis h-6 whitespace-nowrap overflow-hidden w-[200px] lg:w-[300px] xl:w-[350px] ">{songName}</h3>
                        <p className="text-[.8rem] text-[#c4c4c4]">
                            {songArtist && songArtist.map((artist,index)=>
                                <span className='mr-1' key={index}>{artist}{index<songArtist.length-1?",":""}</span>
                            )}
                        </p>
                    </div>
                </div>
                ...
                <div className="">
                    <p className="text-[.9rem] text-[#c4c4c4]">{intoMinutes(sontLength)}</p>
                </div>
            </div>
        </>
    )
